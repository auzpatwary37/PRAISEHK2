...
clean test` → BUILD SUCCESS, **203 tests (1 skipped), 0 failures**, offline. Both invocations are
verified equivalent.
...
* Offline verification: `cd MetaModelCalibration && mvn -o -B clean test`
  → BUILD SUCCESS, **203 tests (1 skipped), 0 failures**, zero compiler diagnostics.
