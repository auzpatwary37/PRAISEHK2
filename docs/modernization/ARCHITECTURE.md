...
Build: `cd MetaModelCalibration && mvn -o -B clean test` → BUILD SUCCESS, 100 tests, 0 failures, offline.
...
* Offline verification: `cd MetaModelCalibration && mvn -o -B clean test`
  → BUILD SUCCESS, 100 tests, 0 failures, zero compiler diagnostics.
