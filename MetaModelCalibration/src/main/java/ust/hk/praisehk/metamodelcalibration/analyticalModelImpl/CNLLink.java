...
public class CNLLink extends AnalyticalModelLink{
	...
	public double getLinkTravelTime(Tuple<Double,Double> timeBean,LinkedHashMap<String,Double>params,LinkedHashMap<String,Double>anaParams) {
		...
		double capacityMultiplier=1;
		if(params.get(CNLSUEModel.CapacityMultiplierName)!=null) {
			capacityMultiplier=params.get(CNLSUEModel.CapacityMultiplierName);
		}
		double capacity=super.getCapacity()*(timeBean.getSecond()-timeBean.getFirst())/3600*capacityMultiplier;
