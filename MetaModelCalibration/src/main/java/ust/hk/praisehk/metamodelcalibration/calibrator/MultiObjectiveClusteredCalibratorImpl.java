...
public class MultiObjectiveClusteredCalibratorImpl extends CalibratorImpl{
	private final Map<String,Set<MeasurementType>> keys;
	public MultiObjectiveClusteredCalibratorImpl(Measurements calibrationMeasurements,String fileLoc,ParamReader pReader,double initialTRRadius,int maxSuccessiveRejection, Map<String,Set<MeasurementType>> keys) {
	...
	public LinkedHashMap<String,Double> generateNewParam(AnalyticalModel sue,Measurements simMeasurements,SimAndAnalyticalGradientCalculator gradFactory, String metaModelType) {
		...
		if(this.iterationNo>0) {
			...
			double CurrentSimObjective=Collections.max(ObjectiveCalculator.calcMultiObjective(this.calibrationMeasurements, this.simMeasurements.get(this.currentParamNo),this.ObjectiveType,keys).values());
			double CurrentMetaModelObjective=Collections.max(ObjectiveCalculator.calcMultiObjective(this.calibrationMeasurements, this.CalcMetaModelPrediction(this.currentParamNo),this.ObjectiveType,keys).values());
			double trialSimObjective=Collections.max(ObjectiveCalculator.calcMultiObjective(this.calibrationMeasurements, this.simMeasurements.get(this.iterationNo),this.ObjectiveType,keys).values());
			double trialMetaModelObjective=Collections.max(ObjectiveCalculator.calcMultiObjective(this.calibrationMeasurements, this.CalcMetaModelPrediction(this.iterationNo),this.ObjectiveType,keys).values());
		...
		if(this.iterationNo>0) {
			if(this.calcAverageMetaParamsChange()<this.minMetaParamChange && !metaModelType.equals(MetaModel.GradientBased_I_MetaModelName)) {
				...
				Map<String, Double> currObj = ObjectiveCalculator.calcMultiObjective(this.calibrationMeasurements, this.simMeasurements.get(this.currentParamNo),this.ObjectiveType,keys);
				Map<String,Double> bstObj = new HashMap<>();
 				for(Entry<String, Set<MeasurementType>> mType:this.keys.entrySet()) {
					AnalyticalModelOptimizer anaOptimizer=new AnalyticalModelOptimizerImpl(new SingleClusteredMultiObjectiveOptimizationFunction(sue, this.calibrationMeasurements, this.metaModels, this.currentParam, this.TrRadius, this.pReader.getInitialParamLimit(),this.ObjectiveType ,metaModelType,this.pReader, this.iterationNo,this.fileLoc,mType.getValue()));
					anaOptimizer.setOptimizerType(AnalyticalModelOptimizer.TROptimizerName);
					this.trialParam=anaOptimizer.performOptimization();
					bstObj.put(mType.getKey(), anaOptimizer.getOptimizationFunction().getMinObj());
				}
 				ClusturedMultiObjOptimDecisionObjective optimFunc = new ClusturedMultiObjOptimDecisionObjective(sue, this.calibrationMeasurements, 
 						this.metaModels, this.currentParam, this.TrRadius, this.pReader.getInitialParamLimit(),
