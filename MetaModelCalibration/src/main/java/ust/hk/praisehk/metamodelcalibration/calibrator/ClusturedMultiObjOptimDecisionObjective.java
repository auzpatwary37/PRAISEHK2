...
public class ClusturedMultiObjOptimDecisionObjective extends SimpleOptimizationFunction{
	private final Map<String,Set<MeasurementType>> keys;
	private final Map<String,Double> bestObj;
	private final Map<String,Double> currentObj;
	private final int calibrationParamSize;
	private final int objectiveSize;
	private final int constrainNo;

	
	protected ClusturedMultiObjOptimDecisionObjective(AnalyticalModel sueAssignment, Measurements realData,
			...
			String MetaModelType, ParamReader pReader, int currentIterNo, String fileLoc,Map<String,Set<MeasurementType>> keys, Map<String, Double> bestObjectives,
	...
	public double[] calcConstrain(double[] x, LinkedHashMap<String, Tuple<Double, Double>> paramLimit) {
		...
		Map<String,Double> obj = ObjectiveCalculator.calcMultiObjective(this.getRealData(), metaMeasurements, super.type,keys);
		
		for(Entry<String, Double> d:obj.entrySet()) {
			k++;
